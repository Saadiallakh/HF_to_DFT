...
best_xgb = xgb.XGBRegressor(**best_params, random_state=42, tree_method='hist', device='cuda')
best_xgb.fit(X_train_numeric_scaled, y_train_scaled)

#-# Save the trained model
model_dir = "./output"

joblib.dump(best_xgb, os.path.join(model_dir, 'xgboost_model.pkl'))
print(f"Trained model with Optuna saved to {os.path.join(model_dir, 'xgboost_model.pkl')}")
 
#-# Predict values on the test data
y_pred_scaled = best_xgb.predict(X_test_numeric_scaled)
