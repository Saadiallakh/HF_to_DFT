...
def objective(trial):
    ...
    """
    #-# Set the regularization hyperparameter range
...
best_lasso = Lasso(**best_params, random_state=42, max_iter=100000)
best_lasso.fit(X_train_numeric_scaled, y_train_scaled)

#-# Save the trained model
model_dir = "./output"

joblib.dump(best_lasso, os.path.join(model_dir, 'lasso_regression_model.pkl'))
print(f"Trained model with Optuna saved to {os.path.join(model_dir, 'lasso_regression_model.pkl')}")
 
#-# Predict values on the test data
y_pred_scaled = best_lasso.predict(X_test_numeric_scaled)
